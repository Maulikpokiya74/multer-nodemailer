...
//     jwt.verify(token, config.jwtsecretkey, async (err, result) => {
//         if (err) {
//             return res.json({ status: false, message: "Something is wrong in Authentication.Please try again.", isAuth: false, data: [] });
//         } else {
//             if (result) {
//                 if (isset(result.id)) {
//                     const getAdminData = await adminModel.getProfileByEmail(result.email);
//                     if (!getAdminData.status) {
//                         return res.json({ status: false, message: "Invalid token or expired!", isAuth: false, data: [] });
//                     }
//                     req.body.id = result.id;
//                     return next();
//                 } else {
